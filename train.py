...
from read_csv import read_mnist
...
def train(config: str):
    ...
    train_dir= config.get('train_dir', './train-data.csv')
    ...
    pics, labels= read_mnist(train_dir)                                         #调用 read_mnist 函数读取训练数据集，并将图像和标签分别存储在 pics 和 labels 中。
    picss= pics.view(-1, 1, 64, 64)                                             # 将 pics 中的每个图像从一维向量（1x784）转换为四维张量（-1x1x28x28），其中 -1 表示自动计算批次大小，1 表示图像的通道数，28x28 表示图像的高度和宽度。
    dataset=torch.utils.data.TensorDataset(picss,labels)
    dataloader=torch.utils.data.DataLoader(dataset, batch_size= batch_size, shuffle= True)     #使用 DataLoader 创建数据加载器：train_loader：训练数据加载器，批量大小为 batch_size，并且每次迭代时会打乱数据。
...
if __name__ == '__main__':
    train(config='C:/Users/Lenovo/Desktop/ai-homework/train_config.yaml')#训练模型,这里记得要改成自己的train_config.yaml路径!!!
